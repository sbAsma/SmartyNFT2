...
class RegistrationForm(FlaskForm):
	...
	def validate_email(self, email):
		...
		if user:
			raise ValidationError('That email was taken, please choose another one')
...
class UpdateAccountForm(FlaskForm):
	...
	competitors = StringField('competitors', 
							validators = [ Length(min=3, max=250) ] )
	email = StringField('Email', 
						validators = [DataRequired(), Email()] )
...
class RequestResetForm(FlaskForm):
	...
	def validate_email(self, email):
		user = User.query.filter_by(email = email.data).first()
		if user is None:
			raise ValidationError('That is no accoutn with that email, you must register first.')
